                    ...
                    <table id="sales-variable" class="table">
                        <thead>
                            <tr>
                                ...
                                <th style="width:50% ; margin-left:-550px"> <?php echo app('translator')->get('translation.Variable Name'); ?> </th>
                                <th style="width:60%"> 

                                    <?php echo app('translator')->get('translation.Variable Label Value'); ?> 
                                    <button type="button" class="btn btn-info btn-sm ml-2" id="checkButton"> 

                                        <?php echo app('translator')->get('translation.check'); ?> </button>
...
document.addEventListener("DOMContentLoaded", function() {
    $('#checkButton').on('click', function() {
        $.ajax({
            url: '<?php echo e(route("sales.data")); ?>',
            type: 'GET',
    ...
    function populateTable(data) {
        ...
        data.forEach(function(item) {
           ...
           // Extract the first two non-empty name:value pairs from variableJson
          /*
          if (Array.isArray(variableJson)) {
                variableJson.forEach(function(variable) {
                    if (variable.value && variableValues.length < 2) {
                        variableValues.push(`${variable.name} : ${variable.value}`);
                    }
                });
            }

            var variableDisplay = variableValues.join(', '); 
            */
...
<script>
    ...
    switch (variable.VariableType) {
        case 'Dates':
            ...
            var mandatorySign = $('<span class="text-danger mr-1">*</span>');
        ...
        $.each(variableLabelValues, function(index, value) {
            ...
            checkbox.on('change', function() {
                ...
                container.find('input[type="checkbox"]:checked').each(function() {
                    if ($(this).val() !== "on") { // Ensure only valid JSON values are added
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u121027207/domains/appcontratti.it/public_html/sales/resources/views/Send-New-Contracts.blade.php ENDPATH**/ ?>